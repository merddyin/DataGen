function character {
    <#
        .SYNOPSIS
        The function is intended to generate a single random character. Type: Base
        ...
        *
        ...
        .EXAMPLE
        $item = (character -Case Upper -ReturnSet Vowel)
        ...
        .EXAMPLE
        $charCount = Get-Random -Minimum 15 -Maximum 22
        $passVal = ""
        for($i = 1; $i -lt $($charCount + 1); $i++){
            $passVal += "$(character)"
        }

        A random number of characters is selected, then the 'character' base generator is called the requisite number of times with no other
        values to generate an entirely random set of characters for a password that will be at least 15 characters long. This would be code
        found in a Mid level generator.
    #>
