...
The reference scenario sets `identity.includeEnvironmentDefaults` to `false`. That keeps pre-existing AD defaults such as `CN=Users`, `CN=Computers`, `CN=Builtin`, `Domain Admins`, and `Administrators` out of the generated create-list so the population step does not collide with normal domain objects.
