...
Compare by path **relative to each export root**, never by full path: the two runs live in different roots, so a full-path comparison finds nothing in common and reports a spurious mismatch. And check that each run actually produced artifacts before trusting the result — two empty directories compare equal, which reads as a pass and is not one.

```powershell
$runA = Resolve-Path .\out\general-enterprise-4242\normalized
$runB = Resolve-Path .\out\general-enterprise-4242-rerun\normalized

function Get-ExportInventory {
  param([string]$Root)
  ...
  if ($files.Count -eq 0) {
    throw "Export root '$Root' contains no artifacts; there is nothing to compare."
  }
  $files | Sort-Object RelativePath
}

$filesA = Get-ExportInventory -Root $runA.Path
$filesB = Get-ExportInventory -Root $runB.Path
...
Two more things to confirm before reading a clean comparison as evidence:

- **Neither root is stale.** A run that fails after writing some artifacts leaves a partial tree behind. Generate into roots you created empty for the comparison, and treat an export command that reported an error as producing no comparable output at all.
- **Both roots came from the runs you think they did.** Re-exporting with a pinned `-ExportedAtUtc` resolves to the same directory each time, so an export that was refused for already existing content leaves the *previous* run's artifacts in place. Check the export commands succeeded rather than assuming the directory contents are fresh.
