...
The DataGen version is on that list for a reason. A release may change what generation emits for the same inputs, including how many values a layer draws from the shared random source, which shifts the values later layers generate even where row counts are unchanged. An equality claim therefore holds within one version, not across an upgrade. v0.14.0 is such a release; see [What's New](/about/whats-new) for what moved and regenerate any export you keep as a fixed reference.
