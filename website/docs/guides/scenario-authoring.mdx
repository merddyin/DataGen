...
- persona-first with `New-SEScenarioFromPersonaPreset`
- guided interactive authoring with `New-SEScenarioWizard`
- compatibility-oriented template-first with `New-SEScenarioFromTemplate`
...
`identity.includeEnvironmentDefaults` controls whether DataGen emits default AD/tenant objects as generated records. Keep it enabled for analysis datasets that should include built-in containers, default accounts, built-in groups, Domain Controllers OU evidence, and Entra Connect sync artifacts. Set it to `false` when using exports to populate an existing AD lab; generated objects can still reference default DNs for intentional deviations without trying to recreate objects that already exist in the domain.
