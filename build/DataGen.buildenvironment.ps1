...
if ($null -eq (Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue)) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        FirstRun = $True
        Force = $False
        ForceInstallModule = $False
        Encoding = 'utf8'
        ModuleToBuild = 'DataGen'
        ModuleVersion = '0.1.0'
        ModuleWebsite = 'https://github.com/merddyin/DataGen.git'
        ModuleCopyright = "(c) $((get-date).Year.ToString()) Topher Whitfield. All rights reserved."
        ModuleLicenseURI = 'https://github.com/merddyin/DataGen/blob/main/LICENSE'
        ModuleTags = 'Tools,Data,Generator' -split ','
        ModuleAuthor = 'Chris Whitfield'
        ModuleDescription = 'Dymanically creates a set of data for use in populating lab environments, POCs, and demo setups'
        ...
        # You should NOT enter this key here but rather manually enter it in the DataGen.buildenvironment.json file with: set-buildenvironment -NugetAPIKey '<key>'
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "DataGen.buildenvironment.json"
