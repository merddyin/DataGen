...
public sealed class WorldQualityAuditServiceTests
{
    ...
    public void Audit_Flags_Guest_Metadata_And_Duplicate_Principals()
    {
        ...
        Assert.Contains(result.Warnings, warning => warning.Contains("duplicate person display names", StringComparison.OrdinalIgnoreCase));
        ...
        Assert.Contains(result.Warnings, warning => warning.Contains("temporal identity evidence", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(result.Warnings, warning => warning.Contains("workstations are missing both assigned-user and directory-account evidence", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(result.Warnings, warning => warning.Contains("servers are missing directory-account evidence", StringComparison.OrdinalIgnoreCase));
        ...
        Assert.True(result.Metrics["duplicate_person_display_names"] > 0);
        Assert.True(result.Metrics["max_person_display_name_repeat"] > 1);
        Assert.True(result.Metrics["duplicate_person_upns"] > 0);
        Assert.True(result.Metrics["duplicate_account_upns"] > 0);
        Assert.True(result.Metrics["accounts_missing_temporal_identity_evidence"] > 0);
        Assert.True(result.Metrics["workstations_missing_identity_evidence"] > 0);
        Assert.True(result.Metrics["servers_missing_directory_account_evidence"] > 0);
        Assert.Contains("CRM", result.Samples["applications"]);
        Assert.Contains("Alex Carter", result.Samples["people"]);
    }
    ...
    public void Audit_Flags_Generic_Naming_And_Undersized_Policy_Surface()
    {
        ...
        Assert.True(result.Metrics["group_policies_without_linked_container"] > 0);
    }
