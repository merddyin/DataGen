...
public sealed class ScenarioValidationTests
{
    ...
    [InlineData(-1)]
    [InlineData(101)]
    public void Validator_Rejects_Legacy_Directory_Identifier_Count_Outside_Bounds(int count)
    {
        ...
        {
            Name = "Invalid Legacy Identifier Count",
            Identity = new IdentityProfile { LegacyDirectoryIdentifierVariantCount = count }
        });

        Assert.False(result.IsValid);
        Assert.Contains(result.Messages, message => message.Code == "identity-legacy-directory-identifier-count");
    }

    [Fact]
    public void Validator_Allows_Small_Secondary_Company_For_Legacy_Directory_Identifiers()
    {
        var services = new ServiceCollection()
            .AddSyntheticEnterpriseCore()
            .BuildServiceProvider();
        ...
        var result = validator.Validate(new ScenarioEnvelope
        {
            Name = "Primary Legacy Identifier Population",
            Identity = new IdentityProfile { LegacyDirectoryIdentifierVariantCount = 3 },
            Companies = new()
            {
        ...
        Assert.DoesNotContain(result.Messages, message => message.Code == "identity-legacy-directory-identifier-population");
    }

    [Fact]
    public void Validator_Rejects_Legacy_Directory_Identifier_Count_Above_Primary_Company_Population()
    {
        var services = new ServiceCollection()
            .AddSyntheticEnterpriseCore()
            .BuildServiceProvider();
        ...
        var result = validator.Validate(new ScenarioEnvelope
        {
            Name = "Oversized Primary Legacy Identifier Population",
            Identity = new IdentityProfile { LegacyDirectoryIdentifierVariantCount = 13 },
            Companies = new()
            {
                new ScenarioCompanyDefinition
                {
        ...
        Assert.False(result.IsValid);
        Assert.Contains(result.Messages, message => message.Code == "identity-legacy-directory-identifier-population");
    }
