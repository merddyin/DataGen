...
A remediation release. Every entry fixes a finding recorded while v0.13.0 was built rather than fixed at the time; no new surface, no new scenario option, and no new export column.

Changes what generation emits — regenerate a dataset generated by v0.13.0:

- Places a department's organizational unit under the unit of the business unit that owns it, so two departments of the same name no longer produce byte-identical distinguished names; both departments keep their names and position tells the two units apart. A department unit's own name is unchanged; only its position moved, so its distinguished name gains a level. The placement is unconditional and unrelated to whether a department name repeats.
- Qualifies groups derived from a department with the owning business unit only where that department name repeats within the company, because every such group shares one container and the name is the only thing that can separate two of them; a world with no repeated department name sees no group name change.
- Makes a business unit's name unique within its company, because nothing above a business unit distinguishes two of them, and a scenario asking for more business units than the catalog can name distinctly now suffixes the surplus as departments already were.
- Gives each department of a repeated name its own organizational unit, its own people and its own department-scoped policy link; both previously resolved a department by name, so the second received neither.
- Adds a tier of `Business Unit Users` organizational units where none existed before, one per business unit directly under `OU=Employees`, in every world; these are new objects rather than renamed or moved ones, so a consumer enumerating organizational units sees a level it has not seen before.
- Moves no account distinguished name, built-in, service and administrative accounts included. An account is held in the location unit for its person's office, and location units did not move.
- Escapes the common name component when building a group's distinguished name, which was previously interpolated raw; account and organizational unit distinguished names already escaped theirs, so this brings groups into line rather than adding escaping to the export. A component holding a comma or another distinguished-name special character would otherwise read as more components than the object has, and the case that exposes is a consumer splitting a distinguished name on commas rather than parsing it. The shipped catalogs do contain values that would require escaping — office city names with commas, and the surnames `Trease,` and `Vernal;Cooper` — but those feed organizational unit and account names, which already escaped, and whether a world draws such a row at all depends on its seed. No department or organization-template name carries one, so the group change is not expected to move a value in a world generated from the shipped catalogs.
- Emits one shape for a security entry held on an organizational unit: every entry now names the unit rather than the container that mirrors it, and `ApplyGroupPolicy` moves to the policy object whose security descriptor is the only one that carries it.
- Always states inheritance scope on an entry held on an organizational unit, as one of three values, the third recording that the source carried no propagation flag and therefore that how far the entry reaches is not known; only entries whose position was deliberately chosen carry a resolved scope. Both rules are enforced at generation time, which fails if any entry names a mirroring container or if any entry on a unit carries no scope.
- Empties the distinguished name of a cloud-joined device account, which previously carried a bare host name while its on-premises sibling carried a real one; a cloud-only object has no position in a directory.
- Describes some entities with two configuration items that disagree about business criticality, with each band stated by a distinct `cmdb_source_records` row — the database's own record on one side and a record it could not reconcile on the other — rather than asserted. A non-authoritative source names the thing by the second name the item already holds, no second item is created unless both sides state a band, and the dedupe contract is intact: a repeated canonical projection of one entity still collapses and no duplicate key is produced. A `Clean` deviation profile emits none of it.
- Corrects a policy parity audit setting from the key `Audit:Credential Validation` and value `Success,Failure`, which belonged to no collection route, to `Audit:Audit Credential Validation` and `Success and Failure`, the key and spelling a Group Policy backup's `audit.csv` produces.
- Records the reproducibility consequence: the configuration management layer now draws more values from the shared random source and runs before two other layers, so their generated values shift in a world with configuration management enabled. Row counts are unchanged and the same scenario, seed and times still reproduce byte for byte within 0.14.0, but an export produced by 0.13.0 is not reproducible from 0.14.0.
- Leaves the normalized export schema at `2.2.0`. No column is added, removed, or moved; what moved is the value carried in columns already exported.

Leaves generated output unchanged:

- Gives `-Overwrite` on `Export-SEEnterpriseWorld` the meaning its callers already believed it had; it was a declared parameter copied into a request object and never read, while shipped example scripts passed it. An occupied export root is now refused without it, `-Overwrite` deletes and recreates the root rather than writing over it in place so a tree can never be a merge of two exports, and an overwrite is refused outright if the root holds any top-level entry DataGen did not export, with the owned-name set computed from the live export descriptors rather than hardcoded.
- Derives the `synthetic_enterprise_export_<timestamp>` directory name from the supplied `-ExportedAtUtc` rather than the wall clock at the moment the command runs, in invariant culture so a non-Gregorian current culture cannot render the year differently; two runs with identical scenario, seed, generation time and export timestamp now resolve to the same path, while a caller who does not pin `-ExportedAtUtc` still gets a fresh directory per run.
- Refuses to report a determinism receipt hash for a candidate that produced no artifacts, and hardens the copy-paste snippet in the repeatable-run walkthrough that reported two runs as equal on the digest of the empty string when each had produced nothing.
- Makes catalog freshness content-addressed rather than timestamp-based: build residue forces regeneration, the output copy is reconciled by hash and fails the build if it differs, and the untracked working copy that shadows the source CSV files is pinned to the canonical artifact by content. The publisher's deliberate last-write-time preservation, which a contract test asserts, is left alone.
- Runs the deterministic-generation harness in CI with a portable working root; it was wired into no pipeline and defaulted to a drive absent from the release workstation, so the property it exercises had no automated coverage. It drives a fake generator, needs no network, and adds about a minute, and its guard against reporting success for a run that produced no artifacts now has a test.
- Moves four test classes from a directory with no project file into the integration test project that was already compiling them through a wildcard include; the integration test count is 33 before and after, with those classes contributing 15 either way.
- Documents that an approved assembly plugin is pinned by a hash of its compiled package, computed by walking the entry point's output directory recursively, into which the build copies DataGen's own assemblies; a DataGen version change therefore invalidates every approved assembly-plugin registration and each must be approved again. Script-plugin approvals carry no package hash and are unaffected, and the rejection diagnostic is mode-aware for that reason.
- Packages only the requested version into the module archive; it compressed the parent staging folder, so an archive built in a working tree with release history carried every version previously staged there and grew by one stale module per past release. A build agent never saw it, because its staging root is empty every time, and the archive layout is unchanged.
- Removes the UTF-8 byte order marks that the scripted 0.13.0 version bump wrote into fifteen files that did not carry one before, including `Directory.Build.props`, which the release preflight parses as XML, and `website/package.json`, which the documentation build parses as JSON.
